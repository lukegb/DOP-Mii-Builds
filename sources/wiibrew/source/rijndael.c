...
static u32 pack(u8 *b) { /* pack bytes into a 32-bit Word */
    return ((u32)b[3]<<24)|((u32)b[2]<<16)|((u32)b[1]<<8)|(u32)b[0];
}

static void unpack(u32 a,u8 *b) { /* unpack bytes from a word */
    b[0]=(u8)a;
    b[1]=(u8)(a>>8);
    b[2]=(u8)(a>>16);
    b[3]=(u8)(a>>24);
...
static u8 bmul(u8 x,u8 y) { /* x.y= AntiLog(Log(x) + Log(y)) */
...
static u8 product(u32 x,u32 y) { /* dot product of two 4-byte arrays */
    u8 xb[4],yb[4];
    unpack(x,xb);
    unpack(y,yb);
...
void gentables(void) { /* generate tables */
    int i;
    u8 y,b[4];
...
//void gkey(int nb,int nk,char *key) { /* blocksize=32*nb bits. Key=32*nk bits */
...
//void encrypt(char *buff) {//changed to remove compile warning
...
void decrypt(unsigned char *buff) {
    ...
    for (i=1;i<Nr;i++) { /* Nr is number of rounds. May be odd. */
        ...
        for (m=j=0;j<Nb;j++,m+=3) { /* This is the time-critical bit */
