...
s32 SYSCONF_Init(void)
{
	...
	fd = ISFS_Open(__sysconf_txt_file,1);
	if(fd < 0) return fd;
...
int __SYSCONF_WriteTxtBuffer(void)
{
	...
	if (ret < 0) {
		if (ISFS_Delete(__sysconf_txt_file) < 0) {
			printf("CAN'T SET ATTR");
			return ret;
		}
		ISFS_CreateFile(__sysconf_txt_file, 0, 3, 3, 3);
	}
...
int __SYSCONF_WriteBuffer(void)
{
	...
	fd = ISFS_Open(__sysconf_file,ISFS_OPEN_WRITE);
	if (fd < 0) {
		printf("CAN'T OPEN");
		return fd;
	}
	...
	ret = ISFS_Write(fd, __sysconf_buffer, 0x4000);
	ISFS_Close(fd);
	if(ret != 0x4000) return SYSCONF_EBADFILE;
...
s32 SYSCONF_SaveChanges(void)
{
	...
	if (ret < 0) {
		printf("I CAN'T WRITEBUFFER");
		return ret;
	}
	
	ret = __SYSCONF_WriteTxtBuffer();
	if (ret < 0) {
		printf("I CAN'T WRITETXTBUFFER");
		return ret;
	}
